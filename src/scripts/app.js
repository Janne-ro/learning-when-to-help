//define main application module and its dependencies
...
app.config(['$routeProvider', '$mdThemingProvider', function($routeProvider, $mdThemingProvider) {
    ...
    //create a wrapper directive for the canvas gauge used in the thank you page
    ...
    return {
        ...
        var opts = {
            ...
            maxValue: scope.max ? parseFloat(scope.max) : 9,
            units: scope.units || '',
            majorTicks: scope.majorTicks ? scope.majorTicks.split(',') : [0,3,6,9],
            value: scope.value || 0,
            valueBox: true,
        ...
        if (scope.underColor){
            opts.colorUnits = scope.underColor;
        }
        if (scope.overColor){
            opts.highlights = [{ from: 0, to: opts.maxValue || 100, color: scope.overColor }];
        }
        if (scope.mask){
            opts.valueInt = false, opts.valueFormat = scope.mask;
        }
